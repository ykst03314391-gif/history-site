...
- **見出し・ボタン文言はストリードに合わせて自動化**：お出かけ＝「宿・チケットを予約する」／ゲーム＝「購入・予約」／その他＝「関連書籍」。個別に変えたいときは任意フィールド `buyLabel` で見出しを上書きできる。
- **開示文もリンク先に合わせる**：Amazon を含むときだけ「Amazonのアソシエイトとして…」の一文を自動付加、含まないとき（楽天トラベル・アソビュー等）は汎用の開示文のみ表示する。
